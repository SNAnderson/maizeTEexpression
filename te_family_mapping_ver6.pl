#te_family_mapping_ver6.pl by sna
...
#SAM file must fist be edited so that all mappings are listed as NH:i:1, and add "real hits" tag (RH:)
#EDITED SAM FILE MUST BE SORTED BY READ NAME!!! 

die "usage: <te_family_mapping_ver6.pl> <HTseq.samout> <sample_id> \n" unless @ARGV == 2;
...
my ($genename, $family, $unique_tename);
my %multi_unsorttest;
...
while(my $line = <$file>){
  ...
  $hit = pop @line;
  $real = pop @line;
  $realhits = substr $real,3; #derived from RH tag added in perl script changing all reads to NH:i:1
  ...
  if ($lastread !~ m/empty/ and $read !~ m/^$lastread$/){ #if the read is different from the last read, decide what to do with the last read
	...
	if ($read_fam =~ m/Gene/){ 
	  ...
	  if ($unique_tehits !~ m/empty/ and $ambig_reads =~ m/$unique_tehits/){ #With paired-end reads, one end could be defined while the other is ambiguous. When this is the case, count towards the defined end
	    ...
	    $family = substr $unique_tename,0,8;
	  ...
	  $family = $read_fam;
	  $unique_1_te++; #1te here means one TE family, not individual element. Should not go to te/gene column of summary file.
    ...
    $famid = "error"; #set to error to find problems 
    $teid = "error"; #set to error to find problems 
    $geneid = "error"; #set to error to find problems 
...
####################################################################################################################
  ...
  if ($hit =~ m/no_feature/){
	...
	if ($it1 !~ m/gene/){
	  $has_te = "yes"; 
	  $famid = substr $it1,0,8;
	  $teid = substr $it1,0,21;
	}
      }
	...
	if (($has_te =~ m/yes/) and ($it2 =~ m/$famid/ or $it2 =~ m/gene/)){
	  push(@matching, $it2);
	  if ($it2 !~ m/gene/){
	    $hitstoTEs++;
	  }
      ...
      $listnum = @features;
      $matchnum = @matching;
      if ($listnum == $matchnum and $has_te =~ m/yes/){  
    ...
    if ($hit =~ m/gene/){
      $read_fam = "Gene";
      $geneid = substr $hit,10,14;
      if ($realhits == 1){
	$gene_hit = $geneid; 
      }
    }
    else{
      $famid = substr $hit,5,8;
      $teid = substr $hit,5,21;
      unless ($read_fam =~ m/empty/){
	if ($read_fam !~ m/$famid/){
	  unless ($read_fam =~ m/Gene/){
	    $read_fam = "Multifam";
...
open ($uniqueout,">",$unique_table) or die "Couldn't open $unique_table: $!";
